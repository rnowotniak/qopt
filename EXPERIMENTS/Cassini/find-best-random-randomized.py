...
while True:
    ile_zaburzac = random.randint(1,len(bounds) - 2)
    do_zaburzenia = range(len(bounds))
    while len(do_zaburzenia) != ile_zaburzac:
        do_zaburzenia.remove(random.choice(do_zaburzenia))
    x = []
    for i in xrange(len(best_known)):
        if i not in do_zaburzenia:
            x.append(best_known[i])
            continue
        while True:
